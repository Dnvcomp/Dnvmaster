...
<li id="li-comment-{{ $item->id }}" class="comment even {{ ($item->user_id == $article->user_id) ? 'bypostauthor odd' : '' }}">
    <div id="comment-{{ $item->id }}" class="comment-container">
        <div class="comment-author vcard">
            ...
            <img alt="" src="https://www.gravatar.com/avatar/{{ $hash }}?=mm&s=75" class="avatar img-circle" height="75" width="75">
            <cite class="fn">{{ $item->user->name or $item->name }}</cite>
        </div>
        <!-- .comment-author .vcard -->
        <div class="comment-meta commentmetadata">
            <div class="intro">
                <div class="commentDate">{{ is_object($item->created_at) ? $item->created_at->format('F d, Y.  H:i') : '' }}</div>
    ...
    @if(isset($com[$item->id]))
        <ul class="children">
            @include(env('MASTER').'.comment',['items'=>$com[$item->id]])
        </ul>
    @endif
</li>
@endforeach
