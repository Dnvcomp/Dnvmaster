                ...
                <div class="blog-list">
                    <div class="row">
                        <div class="col-md-8 col-sm-12 col-xs-12">
                            <img src="{{ asset(env('MASTER')) }}/img/portfolio/{{ $portfolio->img->max }}" style="width: 750px; height: 385px" alt="{{ $portfolio->title }}">
                        </div>
                        ...
                        <ul class="pager">
                            @if($portfolios->lastPage() > 1)
                                @if($portfolios->currentPage() !== 1)
                                    <li><a style="color: #FFFFFF; background-color: #18ba60; font-size: 16px;" href="{{ $portfolios->url(($portfolios->currentPage() - 1)) }}">{{ Lang::get('pagination.previous') }}</a></li>
                                @endif
                                ...
                                @if($portfolios->currentPage() !== $portfolios->lastPage())
                                    <li><a style="color: #FFFFFF; background-color: #18ba60; font-size: 16px;" href="{{ $portfolios->url(($portfolios->currentPage() + 1)) }}">{{ Lang::get('pagination.next') }}</a></li>
                                @endif
                            @endif
                        </ul>
                    </div>
